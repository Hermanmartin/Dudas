...
//* como es el contexto en este ejercicio
function counter() {
...
function cacheFunction(cb) { //aca en CB voy a tener multiplicar que es x*2 
   const cache = {} // creo un objeto para es la memoria caché
  return function(num){ // funcion retornada por otra funcion CLOUSURE
    if(!cache.hasOwnProperty(num)){ //verifica si ya tengo la propiedad  
    }
    cache[num] = cb(num) //* no entiendo bien estas lineas de codigo (estoy creando una propiedad que cache que es el objeto le agrego una propiedad que es num ) como se agregan las propiedad y los valores de las propiedades a los objetos
    return cache[num];
  }
}
...
const funcionMultiplicar = cacheFunction(multiplicar)
console.log(funcionMultiplicar(4))
console.log(funcionMultiplicar(5))
console.log(funcionMultiplicar(3))
//----------------------------------------
